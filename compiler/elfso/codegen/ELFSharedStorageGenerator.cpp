    ...
    TR::AOTMethodHeader* hdr;
     ...
     // for( auto it = ELFDataMap.begin(); it != ELFDataMap.end(); ++it )
      for( auto it = ELFDataMap.begin(); it != ELFDataMap.end(); ++it )
        ...
        uint32_t AotCDBufferSize = hdr->sizeOfSerializedVersion();
        ...
        writeSymbolToFile(fp, 
                          ...
                          hdr->getCompiledCodeStart() ? 1 : SHN_UNDEF, 
...
//========================================================================================================================
 ...
 TR::AOTStorage* ELF::ELFSharedObjectGenerator::self() //suspicious behaviour .. not sure about correctness
   {
   return reinterpret_cast<TR::AOTStorage*> (this);
   }
...
ELF::ELFSharedObjectGenerator::consolidateCompiledCode(uint32_t methodCount, char * filename)
    ...
    ptrStart = ptrEnd = compiledCodeBuffer;
      ...
      for( auto it = ELFDataMap.begin(); it != ELFDataMap.end(); ++it )
        {
        hdr = it->second;
        memcpy(ptrEnd, hdr->getCompiledCodeStart(), hdr->getCompiledCodeSize());
        hdr->setCompiledCodeStart(ptrEnd);
        ptrEnd += hdr->getCompiledCodeSize();
...
ELF::ELFSharedObjectGenerator::calculateAggregateSize()
    ...
    for( NameToHeaderMap::iterator it = ELFDataMap.begin(); it != ELFDataMap.end(); ++it )
        ...
        totalCodeSize += hdr->getCompiledCodeSize();
